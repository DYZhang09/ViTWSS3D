...
@PIPELINES.register_module()
class ObjectRandomSelector(object):
    def __init__(self, select_num, is_random=True) -> None:
    ...
    def __call__(self, input_dict):
        ...
        index = np.arange(gt_labels_3d.shape[0])
        if self.is_random:
            np.random.shuffle(index)
        selected_index = index[:self.select_num]

        input_dict['gt_bboxes_3d'] = gt_bboxes_3d[selected_index]
        input_dict['gt_labels_3d'] = gt_labels_3d[selected_index]
        return input_dict

    def __repr__(self) -> str:
        repr_str = self.__class__.__name__
        repr_str += f'(select_num={self.select_num}, is_random={self.is_random})'
        return repr_str
        

@PIPELINES.register_module()
class OutsideBBoxPointsFilter(object):
    """Filter background points outside the (enlarged) bounding box.
    ...
    def __call__(self, input_dict):
        ...
        enlarge_foreground_masks = enlarge_foreground_masks.max(1)
        valid_masks = enlarge_foreground_masks
