...
class KittiDataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        if self.use_pseudo_label:
            print('''\n Note that using pseudo label for training \n''')
            self.pseudo_results = self.load_results(
                results_file='test_kitti.pkl')
            self.pseudo_label_file_name = [sub_dict['file_name'] for sub_dict in self.pseudo_results]

    '''load pesudo label for unlabel dataset'''
    ...
    def get_ann_info(self, index):
        ...
        # if self.use_pseudo_label:
        #     lidar_name = info['point_cloud']['velodyne_path'].split('/')[-1]
        #     if lidar_name in self.pseudo_label_file_name:
        #         idx = self.pseudo_label_file_name.index(lidar_name)
        #         gt_bboxes_3d = self.pseudo_results[idx]['boxes_3d']
        #         gt_labels_3d = self.pseudo_results[idx]['labels_3d']
        #         #print('gt from prediction', len(gt_bboxes_3d),lidar_name)
        #         gt_names = np.array([self.CLASSES[i] for i in gt_labels_3d])
