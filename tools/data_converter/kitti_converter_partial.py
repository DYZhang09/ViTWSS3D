...
from .kitti_data_utils_partial import WaymoInfoGatherer, get_kitti_image_info
...
def _read_imageset_file(path, generate_label_data=True, generate_full_pseudo_label_data=False, label_ratio=10):
    with open(path, 'r') as f:
        lines = f.readlines()

    inter = label_ratio  # 间隔
    lines_label = lines[0:-1:inter]
    lines_unlabel = []
    for q in lines:
        if q not in lines_label:
            lines_unlabel.append(q)


    '''using the 3D ioutmatch training list'''
    # with open('/data/dkliang/projects/synchronous/baidu_a100_3090/YOLOS3d/data/kitti/ImageSets/train_0.02_3.txt', 'r') as f:
    ...
    print("lengh", len(lines), len(lines_label), len(lines_unlabel), len(lines_label)/len(lines))

    if generate_label_data and not generate_full_pseudo_label_data: #generate label data and instead of full pseudo data
        print(f'\n********************************generate label label dara****************\n')
        return [int(line) for line in lines_label]
    elif not generate_label_data and not generate_full_pseudo_label_data:
        print('\n********************************generate unlabel datadara****************\n')
        return [int(line) for line in lines_unlabel]
    else:
        print('\n********************************generate 100% pseudo label dara****************\n')
...
def create_kitti_info_file(data_path,
                           ...
                           with_plane=False,
                           save_path=None,
                           relative_path=True,generate_label_data=True, generate_full_pseudo_label_data=False,label_ratio=10):
    ...
    train_img_ids = _read_imageset_file(str(imageset_folder / 'train.txt'), generate_label_data=generate_label_data, generate_full_pseudo_label_data=generate_full_pseudo_label_data,label_ratio=label_ratio)
    #val_img_ids = _read_imageset_file(str(imageset_folder / 'val.txt'), generate_label_data = generate_label_data, generate_full_pseudo_label_data=generate_full_pseudo_label_data)
    ...
    kitti_infos_train = get_kitti_image_info(
        ...
        relative_path=relative_path, generate_full_pseudo_label_data = generate_full_pseudo_label_data, label_ratio=label_ratio)
    _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path)

    if generate_label_data and not generate_full_pseudo_label_data:
        filename = save_path / f'{pkl_prefix}_infos_train_1_{label_ratio}.pkl'
    elif not generate_label_data and not generate_full_pseudo_label_data:
        filename = save_path / f'{pkl_prefix}_infos_train_{label_ratio-1}_{label_ratio}.pkl'
    else:
        filename = save_path / f'{pkl_prefix}_infos_train_{label_ratio}_{label_ratio}.pkl'
    ...
    # if generate_label_data and not generate_full_pseudo_label_data:
    #     filename = save_path / f'{pkl_prefix}_infos_val_1_10.pkl'
    # elif not generate_label_data and not generate_full_pseudo_label_data:
    #     filename = save_path / f'{pkl_prefix}_infos_val_9_10.pkl'
    # else:
    #     filename = save_path / f'{pkl_prefix}_infos_val_10_10.pkl'
    #
...
def _create_reduced_point_cloud(data_path,
    ...
    for info in mmcv.track_iter_progress(kitti_infos):
        ...
        if save_path is None:
            save_dir = v_path.parent.parent / (v_path.parent.stem + '_reduced_partial')
...
def create_reduced_point_cloud(data_path,
                               ...
                               save_path=None,
                               with_back=False,
                               generate_full_pseudo_label_data = False,
                               label_ratio = 10
                               ):
    ...
    Args:
        ...
        with_back (bool, optional): Whether to flip the points to back.
            Default: False.
    """
    if generate_full_pseudo_label_data:
        if train_info_path is None:
            print('Using pseudo label to reduced the points')
            train_info_path = Path(data_path) / f'{pkl_prefix}_infos_train_{label_ratio}_{label_ratio}.pkl'
