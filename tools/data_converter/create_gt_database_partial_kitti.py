...
def create_groundtruth_database(dataset_class_name,
    ...
    if database_save_path is None:
        if generate_full_pseudo_label_data:
            database_save_path = osp.join(data_path, f'{info_prefix}_gt_database_partial_{label_ratio}')
        else:
    ...
    if generate_label_data and not generate_full_pseudo_label_data:
        if db_info_save_path is None:
            db_info_save_path = osp.join(data_path,
                                         f'{info_prefix}_dbinfos_train_1_{label_ratio}.pkl')
    elif not generate_label_data and not generate_full_pseudo_label_data:
        if db_info_save_path is None:
            db_info_save_path = osp.join(data_path,
                                         f'{info_prefix}_dbinfos_train_{label_ratio-1}_{label_ratio}.pkl')
    else:
        if db_info_save_path is None:
            db_info_save_path = osp.join(data_path,
                                         f'{info_prefix}_dbinfos_train_{label_ratio}_{label_ratio}.pkl')
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        for i in range(num_obj):
            ...
            if generate_full_pseudo_label_data:
                rel_filepath = osp.join(f'{info_prefix}_gt_database_partial_{label_ratio}', filename)
            else:
