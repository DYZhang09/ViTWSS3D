...
def get_label_path(idx,
                   ...
                   exist_check=True,
                   info_type='label_2',
                   use_prefix_id=False, generate_full_pseudo_label_data = False, label_ratio=10):

    if generate_full_pseudo_label_data:
        info_type = f'label_pseudo_{label_ratio}'
...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        if label_info:
            label_path = get_label_path(idx, path, training, relative_path,generate_full_pseudo_label_data=generate_full_pseudo_label_data,label_ratio=label_ratio)
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path)
            #print(label_path, annotations)
